...
class NavigationInterface():
	def __init__(self):
		...
		self.sub_trig = rospy.Subscriber('~imagery_trigger', PoseStamped, self.callback_trigger)
		...
		self.pub_drop = rospy.Publisher('/release_signal', String, queue_size=10)
	...
	def callback_progress(self, msg_in):
		...
		if self.on_diversion and (msg_in.progress == 1.0):
			...
			#drop
			if self.colour == "Blue":
				self.pub_drop.publish('blue open')
				#wait
				rospy.sleep(rospy.Duration(2))
				self.pub_drop.publish('blue close')				
			...
			if self.colour == "Orange":
				self.pub_drop.publish('orange open')
				#wait
				rospy.sleep(rospy.Duration(2))
				self.pub_drop.publish('orange close')				
				# Send a request to contrail to resume path tracking
				req = SetTrackingRequest()
				req.tracking = req.TRACKING_PATH
				res = self.srv_track(req)
